...
class QueryInterface {
    ...
    if (this.sequelize.options.dialect === 'postgres') {
      ...
      return Promise.all(promises).then(results => {
            ...
            if (!results[enumIdx]) {
              ...
              vals.forEach((value, idx) => {
                ...
                if (enumVals.indexOf(value) === -1) {
                  // when adding multiple value to enum this cause all crash
                  // if (vals[idx + 1]) {
        ...
        return Promise.all(promises)
          ...
          .then(() => {
            return this.updateTable(tableName, attributes, options);
          });
      });
    } else {
      if (!tableName.schema &&
  ...
  updateTable(tableName, attributes, options) {
    ...
    .spread((rows, result) => {
      ...
      .then((existingAttributes) => {
        const promises = [];

        for (var attributeName in attributes) {
          if (existingAttributes[attributeName]) {
            continue;
          }
          promises.push(this.sequelize.query(this.QueryGenerator.addColumnQuery(tableName, attributeName, attributes[attributeName]), options));
        }
        return Promise.all(promises);
      });
    });
